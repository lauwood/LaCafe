...
CubeMap::CubeMap() {
	m_Shader.Initialize("Shaders/Skybox_vs.glsl", "Shaders/Skybox_fs.glsl");
}
...
bool CubeMap::LoadCubeMapSide(GLuint texture, GLenum side_target, const char* file_name) {
	...
	if (!image_data) {
		fprintf(stdout, "CubeMap ERROR: could not load %s\n", file_name);
		return false;
	}
	// non-power-of-2 dimensions check
	if ((x & (x - 1)) != 0 || (y & (y - 1)) != 0) {
		fprintf(stdout, "WARNING: image %s is not power-of-2 dimensions\n", file_name);
	}
